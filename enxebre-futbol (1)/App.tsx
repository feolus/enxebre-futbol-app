...
const App: React.FC = () => {
  ...
  const handleAddPlayer = async (playerData: any, idPhotoFile: File | null, dniFrontFile: File | null, dniBackFile: File | null): Promise<void> => {
    try {
        const newPlayer = await firebaseServices.addPlayer(playerData, idPhotoFile, dniFrontFile, dniBackFile);
        setPlayers(prevPlayers => [...prevPlayers, newPlayer]);
    } catch (error) {
        console.error("Error adding player from App.tsx:", error);
  ...
  const handleCreatePlayerAuthUser = async (player: Player, password: string): Promise<void> => {
    ...
    // Also update loggedInPlayer if it's the same person, though unlikely in coach view
    if (loggedInPlayer && loggedInPlayer.id === updatedPlayer.id) {
