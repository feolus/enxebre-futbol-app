...
export const batchAddPlayers = async (playersData: Partial<Player>[]): Promise<void> => {
    const batch = db.batch();
    playersData.forEach(data => {
        ...
        const playerToAdd: Omit<Player, 'id'> = {
            authUid: '', // No auth user created on import
            photoUrl: `https://picsum.photos/seed/${newPlayerId}/200/200`,
            documents: {},
            ...
            medicalInfo: {
                status: 'Activo',
                notes: '',
                treatments: data.medicalInfo?.treatments || '',
            },
            contactInfo: {
            ...
            parentInfo: {
                fatherNamePhone: data.parentInfo?.fatherNamePhone || '',
                motherNamePhone: data.parentInfo?.motherNamePhone || '',
...
export const createPlayerAuthUser = async (player: Player, password: string): Promise<Player> => {
    let userCredential;
    try {
        userCredential = await secondaryAuth.createUserWithEmailAndPassword(player.contactInfo.email, password);
        const uid = userCredential.user?.uid;
        if (!uid) {
            throw new Error("La creación del usuario de Firebase Auth falló, no se devolvió ningún UID.");
        }
        ...
        const batch = db.batch();
        batch.update(playerRef, { authUid: uid });
        batch.set(userRef, { role: 'player', playerId: player.id });
        await batch.commit();
        ...
        return { ...player, authUid: uid };

    } catch (error) {
        if (userCredential && userCredential.user) {
            try {
                await userCredential.user.delete();
                console.log("Se ha eliminado correctamente el usuario de autenticación parcialmente creado.");
            } catch (deleteError) {
                console.error("Crítico: No se pudo eliminar el usuario de autenticación parcialmente creado. Puede ser necesaria una limpieza manual.", deleteError);
            }
        }
        if (secondaryAuth.currentUser) {
            await secondaryAuth.signOut();
        }
        console.error("Error al crear el usuario de autenticación del jugador:", error);
        throw error;
    }
};
